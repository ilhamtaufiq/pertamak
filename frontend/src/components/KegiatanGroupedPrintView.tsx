...
export function KegiatanGroupedPrintView({ data, month, year }: KegiatanGroupedPrintViewProps) {
    ...
    // Grouping logic: Group by Month then sort by date
    const monthlyGroups = data.reduce((acc, current) => {
        const date = parseISO(current.tanggal);
            ...
            {sortedMonthKeys.length === 0 ? (
                <div className="print-section">
                    <div className="header">
                        <h1>JURNAL KEGIATAN SKP {month ? `BULAN ${format(new Date(2024, parseInt(month) - 1), 'MMMM', { locale: id }).toUpperCase()}` : 'SEMUA BULAN'} {displayYear}</h1>
                ...
                sortedMonthKeys.map((monthKey, idx) => {
                    const monthName = format(new Date(2024, parseInt(monthKey) - 1), 'MMMM', { locale: id }).toUpperCase();
                    const items = monthlyGroups[monthKey].sort((a, b) => new Date(a.tanggal).getTime() - new Date(b.tanggal).getTime());

                    return (
                        <div key={monthKey} className={`print-section ${idx < sortedMonthKeys.length - 1 ? 'page-break' : ''}`}>
                            <div className="header">
