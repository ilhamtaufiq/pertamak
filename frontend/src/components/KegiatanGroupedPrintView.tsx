...
export function KegiatanGroupedPrintView({ data, month, year }: KegiatanGroupedPrintViewProps) {
    ...
    const groupedData = data.reduce((acc, current) => {
        ...
        if (!acc[userId]) {
            acc[userId] = {
                userInfo: current.user?.karyawan || { nama: current.user?.name || 'Unknown', nip: '-', jabatan: 'Petugas' },
                items: []
            };
        }
        acc[userId].items.push(current);
        return acc;
                    ...
                    <div key={userId} className={`print-section ${idx < userIds.length - 1 ? 'page-break' : ''}`}>
                        ...
                        <div className="mt-12 flex justify-end">
                            <div className="text-center w-[300px]">
                                ...
                                <p className="font-bold underline">{group.userInfo.nama}</p>
                                <p>NIP. {group.userInfo.nip || '-'}</p>
                            </div>
                        </div>
                    </div>
                );
